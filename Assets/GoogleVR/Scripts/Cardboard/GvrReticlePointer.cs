...
public class GvrReticlePointer : GvrBasePointer
{
    ...
    protected enum EFuseState
    {
        Inactive, Running, Triggering, Triggered, Done
    }


    protected EFuseState           fuseState;
    protected float                fuseTime;
    protected GvrFuseReticleTarget activeFuseTarget;
    ...
    public override bool TriggerDown
    {
        get
        {
            return base.TriggerDown | fuseState == EFuseState.Triggering;
        }
    }
    ...
    public override bool Triggering
    {
        get
        {
            return base.Triggering | fuseState == EFuseState.Triggering;
        }
    }
    ...
    public override bool TriggerUp
    {
        get
        {
            return base.TriggerUp | fuseState == EFuseState.Triggered;
        }
    }
    ...
    public override void OnPointerEnter(RaycastResult raycastResultResult, bool isInteractive)
    {
        ...
        ProcessFuse(raycastResultResult.gameObject);
    }
    ...
    protected void ProcessFuse(GameObject activeObject)
    {
        GvrFuseReticleTarget fuseTarget = null;
        if (activeObject != null)
        {
            // has the current object we are looking at a fuse target component?
            fuseTarget = activeObject.GetComponentInParent<GvrFuseReticleTarget>();
        }
        if (fuseTarget == null)
        {
            fuseTime = 0;
            fuseState = EFuseState.Inactive;
            activeFuseTarget = null;
        }
        else if (fuseTarget.isActiveAndEnabled)
        {
            if (fuseTarget != activeFuseTarget)
            {
                fuseTime  = 0;
                fuseState = EFuseState.Running;
                activeFuseTarget = fuseTarget;
            }
        }
    }
    ...
    {
        float shaderFuseTime = 0;
        if (fuseState == EFuseState.Running)
        {
            fuseTime += Time.deltaTime;
            if (activeFuseTarget != null)
            {
                shaderFuseTime = Mathf.Clamp(fuseTime / activeFuseTarget.FuseTime, 0, 1);
            }
            if (shaderFuseTime >= 1.0f)
            {
                // fuse done: push trigger button
                fuseState = EFuseState.Triggering;
            }
        }
        else if (fuseState == EFuseState.Triggering)
        {
            // button has been pushed
            fuseState = EFuseState.Triggered;
        }
        else if (fuseState == EFuseState.Triggered)
        {
            fuseState = EFuseState.Done;
        }
        MaterialComp.SetFloat("_FuseTime", shaderFuseTime);
    }
